...
public class BlogServiceImpl extends ServiceImpl<BlogMapper, Blog> implements IBlogService {
    ...
    public Result disLikeBlog (Long id) {
        ...
        // 判断是否点赞
        String key = BLOG_DISLIKED_KEY + id;//
        Double score = srt.opsForZSet().score(key, userId.toString());
        ...
        if (score == null) {
            // 为了减少一次数据库交互
            boolean isSuccess = update()
                    .setSql("disliked = disliked + 1").eq("id", id)
                    .update();
            if (isSuccess) {
                srt.opsForZSet().add(key, userId.toString(), System.currentTimeMillis());
            }
    ...
    @Override
    public boolean updateBlogImage(Long blogId, String imagePath) {
        return blogMapper.updateBlogImage(blogId, imagePath) > 0;
    }
